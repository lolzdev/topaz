 ...
 * such, and must not be misrepresented as being the original software. 3. This
 ...
 * RGFW_IMPLEMENTATION) #define RGFW_IMPLEMENTATION - makes it so source code
 * is included with header
 */
 ...
 * messages and errors when they're found #define RGFW_EGL - (optional) compile
 * with OpenGL functions, allowing you to use to use EGL instead of the native
 * OpenGL functions #define RGFW_DIRECTX - (optional) include integration
 * directX functions (windows only) #define RGFW_VULKAN - (optional) include
 * helpful vulkan integration functions and macros #define RGFW_WEBGPU -
 * (optional) use WebGPU for rendering #define RGFW_NATIVE - (optional) define
 * native RGFW types that use native API structures
 *
 * #define RGFW_X11 (optional) (unix only) if X11 should be used. This option
 * is turned on by default by unix systems except for MacOS #define
 * RGFW_WAYLAND (optional) (unix only) use Wayland. (This can be used with X11)
 * #define RGFW_NO_X11 (optional) (unix only) don't fallback to X11 when using
 * Wayland #define RGFW_NO_LOAD_WGL (optional) (windows only) if WGL should be
 * loaded dynamically during runtime #define RGFW_NO_X11_CURSOR (optional)
 * (unix only) don't use XCursor #define RGFW_NO_X11_CURSOR_PRELOAD (optional)
 * (unix only) use XCursor, but don't link it in code, (you'll have to link it
 * with -lXcursor) #define RGFW_NO_X11_EXT_PRELOAD (optional) (unix only) use
 * Xext, but don't link it in code, (you'll have to link it with -lXext)
 * #define RGFW_NO_LOAD_WINMM (optional) (windows only) use winmm
 ...
 * #define RGFW_NO_IOKIT (optional) (macOS) don't use IOKit #define
 * RGFW_NO_UNIX_CLOCK (optional) (unix) don't link unix clock functions #define
 * RGFW_NO_DWM (windows only) - do not use or link dwmapi #define RGFW_USE_XDL
 * (optional) (X11) if XDL (XLib Dynamic Loader) should be used to load X11
 * dynamically during runtime (must include XDL.h along with RGFW) #define
 * RGFW_COCOA_GRAPHICS_SWITCHING - (optional) (cocoa) use automatic graphics
 * switching (allow the system to choose to use GPU or iGPU) #define
 * RGFW_COCOA_FRAME_NAME (optional) (cocoa) set frame name #define RGFW_NO_DPI
 * - do not calculate DPI (no XRM nor libShcore included) #define
 * RGFW_ADVANCED_SMOOTH_RESIZE - use advanced methods for smooth resizing (may
 * result in a spike in memory usage or worse performance) (eg. WM_TIMER and
 * XSyncValue) #define RGFW_NO_INFO - do not define the RGFW_info struct
 * (without RGFW_IMPLEMENTATION)
 *
 * #define RGFW_ALLOC x  - choose the default allocation function (defaults to
 * standard malloc) #define RGFW_FREE x  - choose the default deallocation
 * function (defaults to standard free) #define RGFW_USERPTR x - choose the
 * default userptr sent to the malloc call, (NULL by default)
 *
 * #define RGFW_EXPORT - use when building RGFW #define RGFW_IMPORT - use when
 * linking with RGFW (not as a single-header)
 *
 ...
 *
 * linux : gcc main.c -lX11 -lXrandr -lGL windows : gcc main.c -lopengl32
 * -lgdi32 macos : gcc main.c -framework Cocoa -framework CoreVideo -framework
 * OpenGL -framework IOKit
 *
 * #define RGFW_IMPLEMENTATION #include "RGFW.h"
 *
 * u8 icon[4 * 3 * 3] = {0xFF, 0x00, 0x00, 0xFF,    0xFF, 0x00, 0x00, 0xFF,
 * 0xFF, 0x00, 0x00, 0xFF,   0xFF, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0xFF,
 * 0xFF, 0xFF, 0x00, 0xFF,     0xFF, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0xFF,
 * 0xFF, 0x00, 0x00, 0xFF};
 *
 * int main() { RGFW_window* win = RGFW_createWindow("name", 100, 100, 500,
 * 500, (u64)0); RGFW_event event;
 *
 * RGFW_window_setExitKey(win, RGFW_escape); RGFW_window_setIcon(win, icon, 3,
 * 3, RGFW_formatRGBA8);
 *
 * while (RGFW_window_shouldClose(win) == RGFW_FALSE) { while
 * (RGFW_window_checkEvent(win, &event)) { if (event.type == RGFW_quit) break;
 * } }
 *
 ...
 * compiling :
 *
 * if you wish to compile the library all you have to do is create a new file
 * with this in it
 *
 * rgfw.c #define RGFW_IMPLEMENTATION #include "RGFW.h"
 *
 * You may also want to add `#define RGFW_EXPORT` when compiling and `#define
 * RGFW_IMPORT`when linking RGFW on it's own: this reduces inline functions and
 * prevents bloat in the object file
 *
 * then you can use gcc (or whatever compile you wish to use) to compile the
 * library into object file
 *
 ...
 *
 * (commands ar and gcc can be replaced with whatever equivalent your system
 * uses)
 *
 * static : ar rcs RGFW.a RGFW.o shared : windows: gcc -shared RGFW.o
 * -lopengl32 -lgdi32 -o RGFW.dll linux: gcc -shared RGFW.o -lX11 -lGL -lXrandr
 * -o RGFW.so macos: gcc -shared RGFW.o -framework CoreVideo -framework Cocoa
...
/*
 * Credits : EimaMei/Sacode : Code review, helped with X11, MacOS and Windows
 * support, Silicon, siliapp.h -> referencing
 *
 * stb : This project is heavily inspired by the stb single header files
 *
 * SDL, GLFW and other online resources : reference implementations
 *
 * contributors : (feel free to put yourself here if you contribute) krisvers
 * (@krisvers) -> code review EimaMei (@SaCode) -> code review Nycticebus
 * (@Code-Nycticebus) -> bug fixes Rob Rohan (@robrohan) -> X11 bugs and
 * missing features, MacOS/Cocoa fixing memory issues/bugs AICDG
 * (@THISISAGOODNAME) -> vulkan support (example) @Easymode -> support,
 * testing/debugging, bug fixes and reviews Joshua Rowe (omnisci3nce) - bug
 * fix, review (macOS) @lesleyrs -> bug fix, review (OpenGL) Nick Porcino
 * (@meshula) - testing, organization, review (MacOS, examples)
 * @therealmarrakesh -> documentation @DarekParodia -> code review (X11) (C++)
 * @NishiOwO -> fix BSD support, fix OSMesa example @BaynariKattu -> code
 * review and documentation Miguel Pinto (@konopimi) -> code review, fix vulkan
 * example @m-doescode -> code review (wayland) Robert Gonzalez (@uni-dos) ->
 * code review (wayland) @TheLastVoyager -> code review @yehoravramenko -> code
 * review (winapi) @halocupcake -> code review (OpenGL) @GideonSerf ->
 * documentation Alexandre Almeida (@M374LX) -> code review (keycodes) Vũ
 * Xuân Trường (@wanwanvxt) -> code review (winapi) Lucas
 * (@lightspeedlucas) -> code review (msvc++) Jeffery Myers (@JeffM2501) ->
 * code review (msvc) Zeni (@zenitsuyo) -> documentation TheYahton (@TheYahton)
 * -> documentation nonexistant_object (@DiarrheaMcgee
 */
...
#if defined(_WIN32) && !defined(RGFW_X11) && !defined(RGFW_UNIX) && !defined(RGFW_WASM) && !defined(RGFW_CUSTOM_BACKEND)	/* (if you're using X11
																 * on windows some how) */
#define RGFW_WINDOWS
#endif
#if defined(RGFW_WAYLAND)
#define RGFW_DEBUG		/* wayland will be in debug mode by default for
				 * now */
#define RGFW_UNIX
#ifdef RGFW_OPENGL
#define RGFW_EGL
...
#ifdef RGFW_WIN95		/* for windows 95 testing (not that it really
				 * works) */
...
#define RGFW_VK_SURFACE NULL
	...
	/*
	 * ! @brief The stucture that contains information about the current
	 * RGFW instance
	 */
	...
	/*
	 * ! @brief The source window stucture for interfacing with the
	 * underlying windowing API (e.g. winapi, wayland, cocoa, etc)
	 */
	...
	/*
	 * ! @brief a stucture for interfacing with the underlying native image
	 * (e.g. XImage, HBITMAP, etc)
	 */
	typedef struct RGFW_nativeImage RGFW_nativeImage;

	/*
	 * ! @brief a stucture for interfacing with pixel data as a renderable
	 * surface
	 */
	typedef struct RGFW_surface RGFW_surface;

	/*
	 * ! a raw pointer to the underlying mouse handle for setting and
	 * creating custom mouse icons
	 */
	...
	typedef RGFW_ENUM(u8, RGFW_key) {
		RGFW_keyNULL = 0,
			...
			RGFW_world1,
			RGFW_world2,
			RGFW_keyLast = 256	/* padding for alignment ~(175
						 * by default) */
	};
	...
	typedef RGFW_ENUM(u8, RGFW_mouseButton) {
		RGFW_mouseLeft = 0,	/* !< left mouse button is pressed */
			RGFW_mouseMiddle,	/* !< mouse-wheel-button is
						 * pressed */
			RGFW_mouseRight,	/* !< right mouse button is
						 * pressed */
	...
	typedef RGFW_ENUM(u8, RGFW_eventType) {
		...
		/*
		 * ! key event note the code of the key pressed is stored in
		 * RGFW_event.key.value !!Keycodes defined at the bottom of the
		 * RGFW_HEADER part of this file!!
		 *
		 * while a string version is stored in
		 * RGFW_event.key.valueString
		 *
		 * RGFW_event.key.mod holds the current mod this means if
		 * CapsLock, NumLock are active or not
		 */
			RGFW_mouseButtonPressed,	/* !< a mouse button has
							 * been pressed
							 * (left,middle,right) */
			RGFW_mouseButtonReleased,	/* !< a mouse button has
							 * been released
							 * (left,middle,right) */
			RGFW_mouseScroll,	/* !< a mouse scroll event */
			RGFW_mousePosChanged,	/* !< the position of the mouse
						 * has been changed */
		/*
		 * ! mouse event note the x and y of the mouse can be found in
		 * the vector, RGFW_x, y
		 *
		 * RGFW_event.button.value holds which mouse button was pressed
		 */
			RGFW_windowMoved,	/* !< the window was moved (by
						 * the user) */
			RGFW_windowResized,	/* !< the window was resized
						 * (by the user), [on WASM this
						 * means the browser was
						 * resized] */
			...
			RGFW_windowRefresh,	/* The window content needs to
						 * be refreshed */

		/*
		 * attribs change event note The event data is sent straight to
		 * the window structure with win->x, win->y, win->w and win->h
		 */
			RGFW_quit,	/* !< the user clicked the quit button */
			RGFW_dataDrop,	/* !< a file has been dropped into the
					 * window */
			RGFW_dataDrag,	/* !< the start of a drag and drop
					 * event, when the file is being
					 * dragged */
		/*
		 * drop data note The x and y coords of the drop are stored in
		 * the vector RGFW_x, y
		 *
		 * RGFW_event.drop.count holds how many files were dropped
		 *
		 * This is also the size of the array which stores all the
		 * dropped file string, RGFW_event.drop.files
		 */
			...
			RGFW_scaleUpdated	/* !< content scale factor
						 * changed */
	};

	/*
	 * ! @brief flags for toggling wether or not an event should be
	 * processed
	 */
	...
	typedef struct RGFW_commonEvent {
		...
		RGFW_window *win;	/* !< the window this event applies too
					 * (for event queue events) */
	} RGFW_commonEvent;
	...
	/*
	 * ! @brief event data for any mouse position event
	...
	typedef struct RGFW_keyEvent {
		...
		RGFW_window *win;	/* !< the window this event applies too
					 * (for event queue events) */
		RGFW_key value;	/* !< the physical key of the event, refers to
				 * where key is physically !!Keycodes defined
				 * at the bottom of the RGFW_HEADER part of
				 * this file!! */
		u8 sym;		/* !< mapped key char of the event */
		RGFW_bool repeat;	/* !< key press event repeated (the key
					 * is being held) */
		RGFW_keymod mod;
	} RGFW_keyEvent;
	...
	typedef union RGFW_event {
		RGFW_eventType type;	/* !< which event has been sent? */
		RGFW_commonEvent common;	/* !< common event data (e.g.)
						 * type and win */
		RGFW_mouseButtonEvent button;	/* !< data for a button
						 * press/release */
		RGFW_mouseScrollEvent scroll;	/* !< data for a mouse scroll */
		RGFW_mousePosEvent mouse;	/* !< data for mouse motion
						 * events */
		RGFW_keyEvent key;	/* !< data for key press/release/hold
					 * events */
		RGFW_dataDropEvent drop;	/* !< dropping a file events */
		RGFW_dataDragEvent drag;	/* data for dragging a file
						 * events */
		RGFW_scaleUpdatedEvent scale;	/* data for monitor scaling
						 * events */
	} RGFW_event;

	/*
	 * ! @!brief codes for for RGFW_the code is stupid and C++ waitForEvent
	 * waitMS -> Allows the function to keep checking for events even after
	 * there are no more events if waitMS == 0, the loop will not wait for
	 * events if waitMS > 0, the loop will wait that many miliseconds after
	 * there are no more events until it returns if waitMS == -1 or waitMS
	 * == the max size of an unsigned 32-bit int, the loop will not return
	 * until it gets another event
	 */
	...
	/*
	 * ! @brief optional bitwise arguments for making a windows, these can
	 * be OR'd together
	 */
	typedef RGFW_ENUM(u32, RGFW_windowFlags) {
		RGFW_windowNoBorder = RGFW_BIT(0),	/* !< the window doesn't
							 * have a border */
			RGFW_windowNoResize = RGFW_BIT(1),	/* !< the window cannot
								 * be resized by the
								 * user */
			RGFW_windowAllowDND = RGFW_BIT(2),	/* !< the window
								 * supports drag and
								 * drop */
			RGFW_windowHideMouse = RGFW_BIT(3),	/* ! the window should
								 * hide the mouse (can
								 * be toggled later on
								 * using
								 * `RGFW_window_showMouse`) */
			RGFW_windowFullscreen = RGFW_BIT(4),	/* !< the window is
								 * fullscreen by default */
			RGFW_windowTransparent = RGFW_BIT(5),	/* !< the window is
								 * transparent (only
								 * properly works on X11
								 * and MacOS, although
								 * it's meant for for
								 * windows) */
			RGFW_windowCenter = RGFW_BIT(6),	/* ! center the window
								 * on the screen */
			RGFW_windowScaleToMonitor = RGFW_BIT(8),	/* ! scale the window to
									 * the screen */
			RGFW_windowHide = RGFW_BIT(9),	/* ! the window is
							 * hidden */
			RGFW_windowMaximize = RGFW_BIT(10),	/* !< maximize the
								 * window on creation */
			RGFW_windowCenterCursor = RGFW_BIT(11),	/* !< center the cursor
								 * to the window on
								 * creation */
			RGFW_windowFloating = RGFW_BIT(12),	/* !< create a floating
								 * window */
			RGFW_windowFocusOnShow = RGFW_BIT(13),	/* !< focus the window
								 * when it's shown */
			RGFW_windowMinimize = RGFW_BIT(14),	/* !< focus the window
								 * when it's shown */
			RGFW_windowFocus = RGFW_BIT(15),	/* !< if the window is
								 * in focus */
			RGFW_windowOpenGL = RGFW_BIT(17),	/* !< create an OpenGL
								 * context (you can also
								 * do this manually with
								 * RGFW_window_createContext_OpenGL) */
			RGFW_windowEGL = RGFW_BIT(18),	/* !< create an EGL
							 * context (you can also
							 * do this manually with
							 * RGFW_window_createContext_EGL) */
	...
	typedef RGFW_ENUM(u8, RGFW_errorCode) {
		RGFW_noError = 0,	/* !< no error */
			RGFW_errOutOfMemory,
			RGFW_errOpenGLContext, RGFW_errEGLContext,	/* !< error with the
									 * OpenGL context */
	...
	/*
	 * ! @brief RGFW_focusIn / RGFW_focusOut, the window who's focus has
	 * changed and if its in focus
	 */
	typedef void (*RGFW_focusfunc) (RGFW_window * win, RGFW_bool inFocus);
	/*
	 * ! @brief RGFW_mouseEnter / RGFW_mouseLeave, the window that changed,
	 * the point of the mouse (enter only) and if the mouse has entered
	 */
	typedef void (*RGFW_mouseNotifyfunc) (RGFW_window * win, i32 x, i32 y, RGFW_bool status);
	/*
	 * ! @brief RGFW_mousePosChanged, the window that the move happened on,
	 * and the new point of the mouse
	 */
	typedef void (*RGFW_mousePosfunc) (RGFW_window * win, i32 x, i32 y, float vecX, float vecY);
	/*
	 * ! @brief RGFW_dataDrag, the window, the point of the drop on the
	 * windows
	 */
	...
	/*
	 * ! @brief RGFW_keyPressed / RGFW_keyReleased, the window that got the
	 * event, the mapped key, the physical key, the string version, the
	 * state of the mod keys, if it was a press (else it's a release)
	 */
	typedef void (*RGFW_keyfunc) (RGFW_window * win, u8 key, u8 sym, RGFW_keymod mod, RGFW_bool repeat, RGFW_bool pressed);
	/*
	 * ! @brief RGFW_mouseButtonPressed / RGFW_mouseButtonReleased, the
	 * window that got the event, the button that was pressed, the scroll
	 * value, if it was a press (else it's a release)
	 */
	typedef void (*RGFW_mouseButtonfunc) (RGFW_window * win, RGFW_mouseButton button, RGFW_bool pressed);
	/*
	 * ! @brief RGFW_mouseScroll, the window that got the event, the x
	 * scroll value, the y scroll value
	 */
	typedef void (*RGFW_mouseScrollfunc) (RGFW_window * win, float x, float y);
	/*
	 * ! @brief RGFW_dataDrop the window that had the drop, the drop data
	 * and the number of files dropped
	 */
	typedef void (*RGFW_dataDropfunc) (RGFW_window * win, char **files, size_t count);
	/*
	 * ! @brief RGFW_scaleUpdated, the window the event was sent to,
	...
	/*
	 * ! @brief monitor mode data | can be changed by the user (with
	 * functions)
	 */
	...
	typedef struct RGFW_monitor {
		...
		float pixelRatio;	/* !< pixel ratio for monitor (1.0 for
					 * regular, 2.0 for hiDPI)  */
	...
	typedef RGFW_ENUM(u8, RGFW_modeRequest) {
		RGFW_monitorScale = RGFW_BIT(0),	/* !< scale the monitor
							 * size */
			RGFW_monitorRefresh = RGFW_BIT(1),	/* !< change the refresh
								 * rate */
			RGFW_monitorRGB = RGFW_BIT(2),	/* !< change the monitor
							 * RGB bits size */
	...
	/*
	 * ! @brief abstract structure for interfacing with the underlying
	 * OpenGL API
	 */
	typedef struct RGFW_glContext RGFW_glContext;

	/*
	 * ! @brief abstract structure for interfacing with the underlying EGL
	 * API
	 */
	...
	typedef RGFW_ENUM(i32, RGFW_glReleaseBehavior) {
		RGFW_glReleaseFlush = 0,	/* !< flush the pipeline will
						 * be flushed when the context
						 * is release */
	...
	typedef RGFW_ENUM(i32, RGFW_glProfile) {
		RGFW_glCore = 0,	/* !< the core OpenGL version, e.g.
					 * just support for that version */
			RGFW_glCompatibility,	/* !< allow compatibility for
						 * older versions of RGFW as
						 * well as the requested
						 * version */
	...
	typedef struct RGFW_glHints {
		i32 stencil;	/* !< set stencil buffer bit size (0 by
				 * default) */
		i32 samples;	/* !< set number of sample buffers (0 by
				 * default) */
		i32 stereo;	/* !< hint the context to use stereoscopic
				 * frame buffers for 3D (false by default) */
		i32 auxBuffers;	/* !< number of aux buffers (0 by default) */
		i32 doubleBuffer;	/* !< request double buffering (true by
					 * default) */
		i32 red, green, blue, alpha;	/* !< set color bit sizes (all
						 * 8 by default) */
		i32 depth;	/* !< set depth buffer bit size (24 by default) */
		i32 accumRed, accumGreen, accumBlue, accumAlpha;	/* !< set accumulated
									 * RGBA bit sizes (all 0
									 * by default) */
		RGFW_bool sRGB;	/* !< request sRGA format (false by default) */
		RGFW_bool robustness;	/* !< request a "robust" (as in
					 * memory-safe) context (false by
					 * default). For more information check
					 * the overview section:
					 * https://registry.khronos.org/OpenGL/extensions/EXT/EXT_robustness.txt */
		RGFW_bool debug;	/* !< request OpenGL debugging (false
					 * by default). */
		RGFW_bool noError;	/* !< request no OpenGL errors (false
					 * by default). This causes OpenGL
					 * errors to be undefined behavior. For
					 * more information check the overview
					 * section:
					 * https://registry.khronos.org/OpenGL/extensions/KHR/KHR_no_error.txt */
		RGFW_glReleaseBehavior releaseBehavior;	/* !< hint how the
							 * OpenGL driver should
							 * behave when changing
							 * contexts
							 * (RGFW_glReleaseNone
							 * by default). For more
		...
		RGFW_glProfile profile;	/* !< set OpenGL API profile
					 * (RGFW_glCore by default) */
		i32 major, minor;	/* !< set the OpenGL API profile
					 * version (by default RGFW_glMajor is
					 * 1, RGFW_glMinor is 0) */
		RGFW_glContext *share;	/* !< Share this OpenGL context with
					 * newly created OpenGL contexts;
					 * defaults to NULL. */
		RGFW_eglContext *shareEGL;	/* !< Share this EGL context
						 * with newly created OpenGL
						 * contexts; defaults to NULL. */
		RGFW_glRenderer renderer;	/* !< renderer to use e.g.
						 * accelerated or software
						 * defaults to accelerated */
	} RGFW_glHints;
	...
	* @{ */
	...
	/*
	 * ! set the window flags (will undo flags if they don't match the old
	 * ones)
	 */
	...
	* @{ */
...
#if defined(RGFW_OPENGL)
	/*
	 * these are native opengl specific functions and will NOT work with
	 * EGL
	 */
	 ...
	 *
	 * NOTE: if you want to switch the graphics context's thread, you have
	 * to run RGFW_window_makeCurrentContext_OpenGL(NULL); on the old
	 * thread then RGFW_window_makeCurrentContext_OpenGL(valid_window) on
	 * the new thread
	 */
...
#include <windows.h>
	...
	struct RGFW_window_src {
		...
		HDC hdc;	/* !< source HDC */
		i32 offsetW, offsetH;	/* !< width and height offset for
					 * window */
		HICON hIconSmall, hIconBig;	/* !< source window icons */
		i32 maxSizeW, maxSizeH, minSizeW, minSizeH, aspectRatioW,
		 aspectRatioH;	/* !< for setting max/min resize (RGFW_WINDOWS) */
#ifdef RGFW_OPENGL
		RGFW_gfxContext ctx;
...
#include <emscripten/key_codes.h>
	...
	typedef struct RGFW_windowInternal {
		/*
		 * ! which key RGFW_window_shouldClose checks. Settting this to
		 * RGFW_keyNULL disables the feature.
		 */
		RGFW_key exitKey;
		i32 lastMouseX, lastMouseY;	/* !< last cusor point (for raw
						 * mouse data) */
		...
		u32 flags;	/* !< windows flags (for RGFW to check and
				 * modify) */
	...
	struct RGFW_window {
		...
		RGFW_windowInternal internal;	/* !< internal window data that
						 * is not specific to the OS */
	...
	typedef struct RGFW_windowState {
		...
		size_t filesCount;
		i32 dropX, dropY;
		RGFW_window *win;	/* !< it's not possible for one of
					 * these events to happen in the frame
					 * that the other event happened */

		RGFW_bool mouseLeave;
		RGFW_window *winLeave;	/* !< if a mouse leaves one widow and
					 * enters the next */
	} RGFW_windowState;
	...
	struct RGFW_info {
		...
		RGFW_event events[RGFW_MAX_EVENTS];	/* A circular buffer
							 * (FIFO), using
							 * eventBottom/Len  */
		...
		XContext context;
		Window helperWindow;
		char *clipboard;	/* for writing to the clipboard
					 * selection */
		...
		RGFW_windowState windowState;	/* ! for checking window state
						 * events */
...
#define RGFW_eventQueuePushEx(eventInit) { RGFW_event e; eventInit; RGFW_eventQueuePush(&e); }
	...
	typedef struct RGFW_colorLayout {
		i32 r, g, b, a;
	} RGFW_colorLayout;
	...
	void RGFW_setInfo(RGFW_info * info) {
		_RGFW = info;
	}
	RGFW_info *RGFW_getInfo(void){
		return _RGFW;
	}


	void *RGFW_alloc(size_t size) {
		return RGFW_ALLOC(size);
	}
	void RGFW_free(void *ptr){
		RGFW_FREE(ptr);
	}

	void RGFW_useWayland(RGFW_bool wayland) {
		RGFW_init();
		_RGFW->useWaylandBool = RGFW_BOOL(wayland);
	}
	RGFW_bool RGFW_usingWayland(void){
		return _RGFW->useWaylandBool;
	}
...
#define RGFW_CHECK_CLIPBOARD() \
	...
	void RGFW_resetKey(void){
		RGFW_MEMSET(_RGFW->keyboard, 0, sizeof(_RGFW->keyboard));
	}
	/*
	...
	/*
	 * These exist to avoid the if (func == NULL) check for (allegedly)
	 * better performance
	 *
...
#include <stdio.h>
...
#ifdef RGFW_DEBUG
		switch (type) {
			case RGFW_typeInfo:RGFW_PRINTF("RGFW INFO (%i %i): %s", type, err, msg);
			break;
			case RGFW_typeError:RGFW_PRINTF("RGFW DEBUG (%i %i): %s", type, err, msg);
			break;
			case RGFW_typeWarning:RGFW_PRINTF("RGFW WARNING (%i %i): %s", type, err, msg);
			break;
			default:break;
		}
	...
	void RGFW_window_checkMode(RGFW_window * win) {
		if (RGFW_window_isMinimized(win) && (win->internal.enabledEvents & RGFW_windowMinimizedFlag)) {
			...
			RGFW_eventQueuePushEx(e.type = RGFW_windowMinimized;
			e.common.win = win);
			...
			RGFW_eventQueuePushEx(e.type = RGFW_windowMaximized;
			e.common.win = win);
			...
			if (RGFW_window_isMaximized(win) == RGFW_FALSE)
				win->internal.flags &= ~(u32) RGFW_windowMaximize;
			RGFW_eventQueuePushEx(e.type = RGFW_windowRestored;
			e.common.win = win);
	...
	size_t RGFW_sizeofInfo(void){
		return sizeof(RGFW_info);
	}
	size_t RGFW_sizeofNativeImage(void){
		return sizeof(RGFW_nativeImage);
	}
	size_t RGFW_sizeofSurface(void){
		return sizeof(RGFW_surface);
	}
	size_t RGFW_sizeofWindow(void){
		return sizeof(RGFW_window);
	}
	size_t RGFW_sizeofWindowSrc(void){
		return sizeof(RGFW_window_src);
	}

	RGFW_window_src *RGFW_window_getSrc(RGFW_window * win) {
		return &win->src;
	}
	RGFW_bool RGFW_window_getPosition(RGFW_window * win, i32 * x, i32 * y) {
		if (x)
			*x = win->x;
		if (y)
			*y = win->y;
	...
	u32 RGFW_window_getFlags(RGFW_window * win) {
		return win->internal.flags;
	}
	RGFW_key RGFW_window_getExitKey(RGFW_window * win) {
		return win->internal.exitKey;
	}
	void RGFW_window_setExitKey(RGFW_window * win, RGFW_key key) {
		win->internal.exitKey = key;
	}
	void RGFW_window_setEnabledEvents(RGFW_window * win, RGFW_eventFlag events) {
		win->internal.enabledEvents = events;
	}
	RGFW_eventFlag RGFW_window_getEnabledEvents(RGFW_window * win) {
		return win->internal.enabledEvents;
	}
	void RGFW_window_setDisabledEvents(RGFW_window * win, RGFW_eventFlag events) {
		RGFW_window_setEnabledEvents(win, (RGFW_allEventFlags) & ~(u32) events);
	}
	void RGFW_window_setEventState(RGFW_window * win, RGFW_eventFlag event, RGFW_bool state) {
		RGFW_setBit(&win->internal.enabledEvents, event, state);
	}
	void *RGFW_window_getUserPtr(RGFW_window * win) {
		return win->userPtr;
	}
	void RGFW_window_setUserPtr(RGFW_window * win, void *ptr){
		win->userPtr = ptr;
	}
...
#include "XDL.h"
	...
	RGFW_window *RGFW_createWindowPtr(const char *name, i32 x, i32 y, i32 w, i32 h, RGFW_windowFlags flags, RGFW_window * win){
		...
		/*
		 * X11 creates the window after the OpenGL context is created
		 * (because of visual garbage), so we have to wait to set the
		 * flags This is required so that way the user can create their
		 * own OpenGL context after RGFW_createWindow is used if a
		 * window is crated, CreateContext will delete the window and
		 * create a new one
		 */
#ifdef RGFW_X11
		 RGFW_window_setFlagsInternal(win, flags, 0);
...
#ifdef RGFW_MACOS
		/*
		 * NOTE: another OpenGL/setFlags related hack, this because OSX
		 * the 'view' class must be setup after the NSOpenGL view is
		 * made AND after setFlags happens
		 */
		...
		if (RGFW_usingWayland()) {
			...
			/*
			 * NOTE: this is a hack so that way wayland spawns a
			 * window, even if nothing is drawn
			 */
	...
	void RGFW_setQueueEvents(RGFW_bool queue) {
		_RGFW->queueEvents = RGFW_BOOL(queue);
	}

	void RGFW_eventQueueFlush(void){
		_RGFW->eventLen = 0;
	}

	void RGFW_eventQueuePush(const RGFW_event * event){
		if (_RGFW->queueEvents == RGFW_FALSE)
			return;
	...
	RGFW_bool RGFW_window_didMouseLeave(RGFW_window * win) {
		return _RGFW->windowState.winLeave == win && _RGFW->windowState.mouseLeave;
	}
	RGFW_bool RGFW_window_didMouseEnter(RGFW_window * win) {
		return _RGFW->windowState.win == win && _RGFW->windowState.mouseEnter;
	}
	RGFW_bool RGFW_window_isMouseInside(RGFW_window * win) {
		return win->internal.mouseInside;
	}

	RGFW_bool RGFW_window_isDataDragging(RGFW_window * win) {
		return RGFW_window_getDataDrag(win, (i32 *) NULL, (i32 *) NULL);
	}
	RGFW_bool RGFW_window_didDataDrop(RGFW_window * win) {
		return RGFW_window_getDataDrop(win, (const char ***)NULL, (size_t *) NULL);
	}


	RGFW_bool RGFW_window_getDataDrag(RGFW_window * win, i32 * x, i32 * y) {
		if (_RGFW->windowState.win != win || _RGFW->windowState.dataDragging == RGFW_FALSE)
			return RGFW_FALSE;
		if (x)
	...
	RGFW_bool RGFW_window_getDataDrop(RGFW_window * win, const char ***files, size_t * count){
		if (_RGFW->windowState.win != win || _RGFW->windowState.dataDrop == RGFW_FALSE)
			return RGFW_FALSE;
		if (files)
			*files = (const char **)_RGFW->files;
		if (count)
			*count = _RGFW->windowState.filesCount;
		return RGFW_TRUE;
	}
	...
	RGFW_bool RGFW_window_isKeyPressed(RGFW_window * win, RGFW_key key) {
		return RGFW_isKeyPressed(key) && RGFW_window_isInFocus(win);
	}
	RGFW_bool RGFW_window_isKeyDown(RGFW_window * win, RGFW_key key) {
		return RGFW_isKeyDown(key) && RGFW_window_isInFocus(win);
	}
	RGFW_bool RGFW_window_isKeyReleased(RGFW_window * win, RGFW_key key) {
		return RGFW_isKeyReleased(key) && RGFW_window_isInFocus(win);
	}

	RGFW_bool RGFW_window_isMousePressed(RGFW_window * win, RGFW_mouseButton button) {
		return RGFW_isMousePressed(button) && RGFW_window_isInFocus(win);
	}
	RGFW_bool RGFW_window_isMouseDown(RGFW_window * win, RGFW_mouseButton button) {
		return RGFW_isMouseDown(button) && RGFW_window_isInFocus(win);
	}
	RGFW_bool RGFW_window_isMouseReleased(RGFW_window * win, RGFW_mouseButton button) {
		return RGFW_isMouseReleased(button) && RGFW_window_isInFocus(win);
	}



#ifndef RGFW_X11
	void *RGFW_getDisplay_X11(void){
		return NULL;
	}
	u64 RGFW_window_getWindow_X11(RGFW_window * win) {
		RGFW_UNUSED(win);
		return 0;
	}
#endif
...
#ifndef RGFW_WAYLAND
	struct wl_display *RGFW_getDisplay_Wayland(void){
		return NULL;
	}
	struct wl_surface *RGFW_window_getWindow_Wayland(RGFW_window * win) {
		RGFW_UNUSED(win);
		return NULL;
	}
#endif
...
#ifndef RGFW_WINDOWS
	void *RGFW_window_getHWND(RGFW_window * win) {
		RGFW_UNUSED(win);
		return NULL;
	}
	void *RGFW_window_getHDC(RGFW_window * win) {
		RGFW_UNUSED(win);
		return NULL;
	}
#endif
	...
	void *RGFW_window_getWindow_OSX(RGFW_window * win) {
		RGFW_UNUSED(win);
		return NULL;
	}
#endif
	...
	RGFW_bool RGFW_window_isHoldingMouse(RGFW_window * win) {
		return RGFW_BOOL(win->internal.holdMouse);
	}
	...
	RGFW_bool RGFW_window_isFullscreen(RGFW_window * win) {
		return RGFW_BOOL(win->internal.flags & RGFW_windowFullscreen);
	}
	RGFW_bool RGFW_window_allowsDND(RGFW_window * win) {
		return RGFW_BOOL(win->internal.flags & RGFW_windowAllowDND);
	}
	...
	void RGFW_window_focusLost(RGFW_window * win) {
		...
		for (key = 0; key < RGFW_keyLast; key++) {
			if (RGFW_isKeyDown((u8) key) == RGFW_FALSE)
				continue;
...
#include <time.h>
	...
	/*
	 * graphics API specific code (end of generic code) starts here
	 */
...
#define RGFW_DEFAULT_GL_HINTS { \
	...
	void RGFW_setGlobalHints_OpenGL(RGFW_glHints * hints) {
		RGFW_globalHints_OpenGL = hints;
	}
	...
	RGFW_glContext *RGFW_window_getContext_OpenGL(RGFW_window * win) {
		if (win->src.gfxType & RGFW_windowEGL)
			return NULL;
	...
	void RGFW_window_deleteContext_OpenGL(RGFW_window * win, RGFW_glContext * ctx) {
		...
		if (win->src.gfxType & RGFW_gfxOwnedByRGFW)
			RGFW_FREE(ctx);
	}
...
const char *extensions = ((const char *(*) (u32))RGFW_glGetString) (RGFW_GL_EXTENSIONS);
	...
	RGFW_bool RGFW_extensionSupported_OpenGL(const char *extension, size_t len){
		if (RGFW_extensionSupported_base(extension, len))
			return RGFW_TRUE;
	...
	RGFW_window *RGFW_getCurrentWindow_OpenGL(void){
		return _RGFW->current;
	}
	void RGFW_attribStack_init(RGFW_attribStack * stack, i32 * attribs, size_t max) {
...
#include <wayland-egl.h>
	...
	void *RGFW_getDisplay_EGL(void){
		return _RGFW->EGL_display;
	}
	void *RGFW_eglContext_getSourceContext(RGFW_eglContext * ctx) {
		return ctx->ctx;
	}
	void *RGFW_eglContext_getSurface(RGFW_eglContext * ctx) {
		return ctx->surface;
	}
	struct wl_egl_window *RGFW_eglContext_wlEGLWindow(RGFW_eglContext * ctx) {
		return ctx->eglWindow;
	}
	...
	RGFW_bool RGFW_loadEGL(void){
		...
		for (size_t i = 0; i < sizeof(libNames) / sizeof(libNames[0]); ++i) {
			...
			if (RGFW_eglLibHandle) {
				...
				if (lib != NULL)
					 RGFW_MEMCPY(&RGFW_eglGetProcAddress, &lib, sizeof(PFNEGLGETPROCADDRESSPROC));
				 break;
			}
#endif
		}
...
#define EGL_OPENGL_ES1_BIT 0x1
...

		{
			...
			if (hints->profile == RGFW_glES) {
				switch (hints->major) {
					case 1:RGFW_attribStack_pushAttrib(&stack, EGL_OPENGL_ES1_BIT);
					break;
					case 2:RGFW_attribStack_pushAttrib(&stack, EGL_OPENGL_ES2_BIT);
					break;
					case 3:RGFW_attribStack_pushAttrib(&stack, EGL_OPENGL_ES3_BIT);
					break;
					default:break;
				}
			} else {
		...
		if (_RGFW->useWaylandBool == RGFW_FALSE) {
			/*
			 * This is required so that way the user can create
			 * their own OpenGL context after RGFW_createWindow is
			 * used
			 */
			XVisualInfo * result;
			XVisualInfo desired;
			...
			if (visualID) {
				...
				result = XGetVisualInfo(_RGFW->display, VisualIDMask, &desired, &count);
				 ...
				 RGFW_sendDebugInfo(RGFW_typeError, RGFW_errEGLContext, "Failed to fetch a valid EGL VisualID");

			if (result == NULL || count == 0) {
				if (win->src.window == 0) {
					/*
					 * try to create a EGL context anyway
					 * (this will work if you're not using
					 * a NVidia driver)
					 */
...
#define EGL_CONTEXT_RELEASE_BEHAVIOR_FLUSH_KHR 0x2098
	...
	RGFW_eglContext *RGFW_window_getContext_EGL(RGFW_window * win) {
		if (win->src.gfxType == RGFW_windowOpenGL)
			return NULL;
	...
	RGFW_window *RGFW_getCurrentWindow_EGL(void){
		return _RGFW->current;
	}
...
#include <unistd.h>
	...
	/*
	 * Sadly we have to use magic linux keycodes We can't use X11
	 * functions, because that breaks Wayland, but they use the same
	 * keycodes so there's no use redeffing them We can't use linux enums,
	 * because the headers don't exist on BSD
	 */
...
#include <limits.h>		/* for data limits (mainly used in drag and
				 * drop functions) */
#include <poll.h>
...
#define XcursorImageDestroy XcursorImageDestroySRC
	...
	RGFW_format RGFW_XImage_getFormat(XImage * image) {
		switch (image->bits_per_pixel) {
			...
			if (image->red_mask == 0x00FF0000 && image->green_mask == 0x0000FF00 && image->blue_mask == 0x000000FF)
				return RGFW_formatARGB8;	/* ambiguous without
								 * alpha */
			break;
		}
...
#define RGFW_PROC_DEF(proc, name) if (name##SRC == NULL && proc != NULL) { \
	...
	void RGFW_window_getVisual(XVisualInfo * visual, RGFW_bool transparent) {
		...
		if (transparent) {
			XMatchVisualInfo(_RGFW->display, DefaultScreen(_RGFW->display), 32, TrueColor, visual);	/* !< for RGBA
														 * backgrounds */
			if (visual->depth != 32)
	...
	void RGFW_XCreateWindow(XVisualInfo visual, const char *name, RGFW_windowFlags flags, RGFW_window * win){
		...
		if (_RGFW->instName == NULL)
			hint.res_name = (char *)name;
		else
			 hint.res_name = (char *)_RGFW->instName;
		...
		if (flags & RGFW_windowScaleToMonitor)
			 RGFW_window_scaleToMonitor(win);
#endif
		 XSelectInput(_RGFW->display, (Drawable) win->src.window, event_mask);	/* !< tell X11 what
											 * events we want */

		/*
		 * make it so the user can't close the window until the program
		 * does
		 */
		...
		/* set the background */
		 ...
		 XMoveWindow(_RGFW->display, (Drawable) win->src.window, win->x, win->y);	/* !< move the window to
												 * it's proper cords */

		if (flags & RGFW_windowAllowDND) {	/* init drag and drop
							 * atoms and turn on
							 * drag and drop for
							 * this window */
			...
			const u8 version = 5;
			 ...
			 XChangeProperty(_RGFW->display, win->src.window,
					 XdndAware, 4, 32,
					 PropModeReplace, &version, 1);	/* !< turns on drag and
									 * drop */
		}
	...
	void RGFW_XHandleEvent(void){
		...
		switch (E.type) {
			...
			case GenericEvent:{
				...
				if (win == NULL)
					return;
				if (!(win->internal.enabledEvents & RGFW_BIT(RGFW_mousePosChanged)))
					 return;

				/*
				 * MotionNotify is used for mouse events if the
				 * mouse isn't held
				 */
				...
				if (E.xcookie.evtype == XI_RawMotion) {
					...
					/*
					 * check if relative motion data exists
		...
		/*
		 * Repeated key presses are sent as a release followed by
		 * another press at the same time. We want to convert that into
		 * a single key press event with the repeat flag set
		 */
		...
		switch (E.type) {
			case KeyPress:{
				if (!(win->internal.enabledEvents & RGFW_keyPressedFlag))
					return;
			...
			case KeyRelease:{
				if (!(win->internal.enabledEvents & RGFW_keyReleasedFlag))
					return;
			...
			if (E.xbutton.button >= Button4 && E.xbutton.button <= 7) {
				if (!(win->internal.enabledEvents & RGFW_mouseScrollFlag))
					return;
				event.type = RGFW_mouseScroll;
			} else {
				if (!(win->internal.enabledEvents & RGFW_mouseButtonPressedFlag) || E.xbutton.button > RGFW_mouseFinal)
					return;
				event.type = RGFW_mouseButtonPressed;
			}

			switch (E.xbutton.button) {
				case Button1:event.button.value = RGFW_mouseLeft;
				break;
				case Button2:event.button.value = RGFW_mouseMiddle;
				break;
				case Button3:event.button.value = RGFW_mouseRight;
				break;
				case Button4:event.scroll.y = 1.0;
				break;
				case Button5:event.scroll.y = -1.0;
				break;
				case 6:event.scroll.x = 1.0f;
				break;
				case 7:event.scroll.x = -1.0f;
				break;
				default:
			...
			if (E.xbutton.button >= Button4 && E.xbutton.button <= 7)
				break;
			if (!(win->internal.enabledEvents & RGFW_mouseButtonReleasedFlag) || E.xbutton.button > RGFW_mouseFinal)
				return;
			...
			if (win->internal.holdMouse)
				return;
			if (!(win->internal.enabledEvents & RGFW_mousePosChangedFlag))
				return;
			...
			case Expose:{
				if (!(win->internal.enabledEvents & RGFW_windowRefreshFlag))
					return;
			...
			break;
			case MapNotify:case UnmapNotify:RGFW_window_checkMode(win);
			break;
			case ClientMessage:{
				RGFW_LOAD_ATOM(WM_DELETE_WINDOW);
				...
				if (win->internal.enabledEvents & RGFW_dataDragFlag)
					return;
			...
			if (!(win->internal.enabledEvents & RGFW_focusInFlag))
				return;
			...
			break;
			case FocusOut:
			if (!(win->internal.enabledEvents & RGFW_focusOutFlag))
				return;
			...
			case EnterNotify:{
				...
				if (!(win->internal.enabledEvents & RGFW_mouseEnterFlag))
					return;
			...
			case LeaveNotify:{
				...
				if (!(win->internal.enabledEvents & RGFW_mouseLeaveFlag))
					return;
			...
			case ConfigureNotify:{
				...
				if (E.xconfigure.width != win->src.w || E.xconfigure.height != win->src.h) {
					...
					if (!(win->internal.enabledEvents & RGFW_windowResizedFlag))
						return;
				...
				if (E.xconfigure.x != win->src.x || E.xconfigure.y != win->src.y) {
					...
					if (!(win->internal.enabledEvents & RGFW_windowMovedFlag))
						return;
	...
	RGFW_bool RGFW_FUNC(RGFW_window_isFloating) (RGFW_window * win) {
		...
		unsigned long i;
		for (i = 0; i < nitems; i++)
			if (prop_return[i] == _NET_WM_STATE_ABOVE)
				return RGFW_TRUE;
	...
	void RGFW_FUNC(RGFW_writeClipboard) (const char *text, u32 textLen){
		...
		/*
		 * request ownership of the clipboard section and request to
		 * convert it, this means its our job to convert it
		 */
	...
	RGFW_bool RGFW_FUNC(RGFW_window_createContextPtr_OpenGL) (RGFW_window * win, RGFW_glContext * context, RGFW_glHints * hints) {
		...
		/*
		 * This is required so that way the user can create their own
		 * OpenGL context after RGFW_createWindow is used
		 */
		if (win->src.window)
			 RGFW_window_closePlatform(win);
		...
		switch (hints->profile) {
			case RGFW_glES:mask |= GLX_CONTEXT_ES_PROFILE_BIT_EXT;
			break;
			case RGFW_glCompatibility:mask |= GLX_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB;
			break;
			case RGFW_glCore:mask |= GLX_CONTEXT_CORE_PROFILE_BIT_ARB;
			break;
		...
		i32 flags = 0;
		if (hints->debug)
			 flags |= GLX_CONTEXT_FLAGS_ARB;
		if (hints->robustness && RGFW_extensionSupportedPlatform_OpenGL(robustStr, sizeof(robustStr)))
	...
	RGFW_proc RGFW_FUNC(RGFW_getProcAddress_OpenGL) (const char *procname){
		return glXGetProcAddress((u8 *) procname);
	}
	...
	void *RGFW_FUNC(RGFW_getCurrentContext_OpenGL) (void){
		return glXGetCurrentContext();
	}
	void RGFW_FUNC(RGFW_window_swapBuffers_OpenGL) (RGFW_window * win) {
		RGFW_ASSERT(win->src.ctx.native);
		glXSwapBuffers(_RGFW->display, win->src.ctx.native->window);
	}
...
#define RGFW_FREE_LIBRARY(x) if (x != NULL) dlclose(x); x = NULL;
		/*
		 * to save the clipboard on the x server after the window is
		 * closed
		 */
		...
		XCloseDisplay(_RGFW->display);	/* !< kill connection to the x
						 * server */
...
#define RGFW_FUNC(func) func
	...
	/*
	 * Wayland TODO: (out of date) - fix RGFW_keyPressed lock state
	 *
	 * RGFW_windowMoved, 		the window was moved (by the user)
	 * RGFW_windowRefresh	 	The window content needs to be refreshed
	 *
	 * RGFW_dataDrop 				a file has been dropped into
	 * the window RGFW_dataDrag
	 *
	 * - window args: #define RGFW_windowNoResize	 			the
	 * window cannot be resized  by the user #define RGFW_windowAllowDND
...
#include <fcntl.h>
	...
	struct wl_display *RGFW_getDisplay_Wayland(void){
		return _RGFW->wl_display;
	}
	struct wl_surface *RGFW_window_getWindow_Wayland(RGFW_window * win) {
		return win->src.surface;
	}
...
#include "xdg-output-unstable-v1.h"
	...
	static void RGFW_wl_setOpaque(RGFW_window * win) {
		...
		if (!wl_region)
			 return;
		//return if no region was created
	...
	static void RGFW_wl_xdg_surface_configure_handler(void *data, struct xdg_surface *xdg_surface,
		...
		if (win->src.resizing) {
			...
			if (!win->src.maximized && win->internal.enabledEvents & RGFW_windowResizedFlag) {
				RGFW_eventQueuePushEx(e.type = RGFW_windowResized;
				e.common.win = win);
				RGFW_windowResizedCallback(win, win->w, win->h);
			}
	...
	static void RGFW_wl_xdg_toplevel_close_handler(void *data, struct xdg_toplevel *toplevel){
		...
		if (!win->internal.shouldClose) {
			RGFW_eventQueuePushEx(e.type = RGFW_quit;
			e.common.win = win);
			RGFW_window_setShouldClose(win, RGFW_TRUE);
			RGFW_windowQuitCallback(win);
		}
	...
	static void RGFW_wl_xdg_decoration_configure_handler(void *data,
		...
		RGFW_window *win = (RGFW_window *) data;
		 RGFW_UNUSED(zxdg_toplevel_decoration_v1);
	...
	static void RGFW_wl_relative_pointer_motion(void *data, struct zwp_relative_pointer_v1 *zwp_relative_pointer_v1,
		...
		RGFW_UNUSED(zwp_relative_pointer_v1);
		RGFW_UNUSED(time_hi);
		RGFW_UNUSED(time_lo);
		RGFW_UNUSED(dx_unaccel);
		RGFW_UNUSED(dy_unaccel);
	...
	static void RGFW_wl_pointer_locked(void *data, struct zwp_locked_pointer_v1 *zwp_locked_pointer_v1){
		...
		RGFW_window *win = RGFW->mouseOwner;
		 ...
		 wl_pointer_set_cursor(RGFW->wl_pointer, RGFW->mouse_enter_serial, NULL, 0, 0);
		//draw no cursor
	}
	...
	static void RGFW_wl_pointer_button(void *data, struct wl_pointer *pointer, u32 serial, u32 time, u32 button, u32 state){
		...
		if (!(win->internal.enabledEvents & (RGFW_BIT(RGFW_mouseButtonReleased - RGFW_BOOL(state)))))
			 return;
		u32 b = (button - 0x110);
	...
	static void RGFW_wl_pointer_axis(void *data, struct wl_pointer *pointer, u32 time, u32 axis, wl_fixed_t value){
		...
		if (!(win->internal.enabledEvents & (RGFW_BIT(RGFW_mouseScroll))))
			 return;
	...
	static void RGFW_wl_keyboard_enter(void *data, struct wl_keyboard *keyboard, u32 serial, struct wl_surface *surface, struct wl_array *keys){
		...
		//is set when RGFW_window_minimize is called;
		if the minimize button is
		// pressed this flag is not set since there is no event to listen for
		if (win->src.minimized == RGFW_TRUE)
			win->src.minimized = RGFW_FALSE;

		 win->internal.inFocus = RGFW_TRUE;
		 RGFW_eventQueuePushEx(e.type = RGFW_focusIn;
		 e.common.win = win);
	...
	static void RGFW_wl_keyboard_leave(void *data, struct wl_keyboard *keyboard, u32 serial, struct wl_surface *surface){
		...
		if (!(win->internal.enabledEvents & RGFW_focusOutFlag))
		 ...
		 RGFW_eventQueuePushEx(e.type = RGFW_focusOut;
		 e.common.win = win);
	...
	static void RGFW_wl_keyboard_key(void *data, struct wl_keyboard *keyboard, u32 serial, u32 time, u32 key, u32 state){
		...
		RGFW_info *RGFW = (RGFW_info *) data;
		if (RGFW->kbOwner == NULL)
			return;

		RGFW_window *RGFW_key_win = RGFW->kbOwner;
		if (!(RGFW_key_win->internal.enabledEvents & (RGFW_BIT(RGFW_keyPressed + state))))
			 return;
	...
	static void RGFW_wl_output_set_scale(void *data, struct wl_output *wl_output, int32_t factor){
		...
		RGFW_monitor * monitor = &((RGFW_monitorNode *) data)
			->mon;
	...
	void RGFW_FUNC(RGFW_releaseCursor) (RGFW_window * win) {
		RGFW_ASSERT(win);
		//compositor has no support or window is not locked
		do
			nothing
			if (_RGFW->constraint_manager == NULL || _RGFW->relative_pointer_manager == NULL)
				return;
		...
		_RGFW->mouseOwner = win;
		//unhold mouse sets this to null;
		set it back
		}

		void RGFW_FUNC(RGFW_captureCursor) (RGFW_window * win) {
			RGFW_ASSERT(win);
			//compositor has no support or window already is locked
			do
				nothing
				if (_RGFW->constraint_manager == NULL || _RGFW->relative_pointer_manager == NULL)
					return;
			...
			RGFW_window *RGFW_FUNC(RGFW_createWindowPlatform) (const char *name, RGFW_windowFlags flags, RGFW_window * win){
				...
				/*
				 * compositor supports both SSD & CSD So choose
				 * accordingly
				 */
			...
			void RGFW_FUNC(RGFW_window_setFullscreen) (RGFW_window * win, RGFW_bool fullscreen) {
				...
				if (fullscreen) {
					...
					win->internal.oldW = win->w;
					win->internal.oldH = win->h;
					xdg_toplevel_set_fullscreen(win->src.xdg_toplevel, NULL);
			...
			RGFW_bool RGFW_FUNC(RGFW_window_setIconEx) (RGFW_window * win, u8 * data, i32 w, i32 h, RGFW_format format, RGFW_icon type) {
				...
				if (_RGFW->icon_manager == NULL || w != h)
					return RGFW_FALSE;
			...
			RGFW_bool RGFW_FUNC(RGFW_window_setMouseStandard) (RGFW_window * win, u8 mouse) {
				...
				if (mouse > RGFW_mouseIconCount - 1)
					 return RGFW_FALSE;
			...
			void RGFW_FUNC(RGFW_window_show) (RGFW_window * win) {
				...
				if (win->internal.flags & RGFW_windowFocusOnShow)
					RGFW_window_focus(win);
				/*
				 * wl_surface_attach(win->src.surface, win->x,
				 * win->y, win->w, win->h, 0, 0);
				 */
				wl_surface_commit(win->src.surface);
			}
			...
			void RGFW_FUNC(RGFW_window_deleteContextPtr_OpenGL) (RGFW_window * win, RGFW_glContext * ctx) {
				RGFW_window_deleteContextPtr_EGL(win, &ctx->egl);
				win->src.ctx.native = NULL;
			}
			...
			void *RGFW_FUNC(RGFW_getCurrentContext_OpenGL) (void){
				return RGFW_getCurrentContext_EGL();
			}
			void RGFW_FUNC(RGFW_window_swapBuffers_OpenGL) (RGFW_window * win) {
				...
				switch (message) {
					...
					case WM_ACTIVATE:{
						...
						if ((win->internal.enabledEvents & (RGFW_BIT(RGFW_focusIn - inFocus)))) {
							RGFW_eventQueuePushEx(e.type = (RGFW_eventType) ((u8) RGFW_focusOut - inFocus);
							e.common.win = win);
					...
					case WM_DPICHANGED:{
						...
						if (!(win->internal.enabledEvents & RGFW_scaleUpdatedFlag))
							 return DefWindowProcW(hWnd, message, wParam, lParam);;
						 RGFW_scaleUpdatedCallback(win, scaleX, scaleY);
						 RGFW_eventQueuePushEx(e.type = RGFW_scaleUpdated;
						 e.scale.x = scaleX;
						 e.scale.y = scaleY;
						 e.common.win = win);
						 return DefWindowProcW(hWnd, message, wParam, lParam);
					...
					case WM_PAINT:{
						...
						PAINTSTRUCT ps;
						 BeginPaint(hWnd, &ps);
						 RGFW_eventQueuePushEx(e.type = RGFW_windowRefresh;
						 e.common.win = win);
						 RGFW_windowRefreshCallback(win);
						 EndPaint(hWnd, &ps);
...
#ifdef RGFW_ADVANCED_SMOOTH_RESIZE
					case WM_ENTERSIZEMOVE:SetTimer(win->src.window, 1, USER_TIMER_MINIMUM, NULL);
					break;
					case WM_EXITSIZEMOVE:KillTimer(win->src.window, 1);
					break;
					case WM_TIMER:
					if (!(win->internal.enabledEvents & RGFW_windowRefreshFlag))
						return DefWindowProcW(hWnd, message, wParam, lParam);
					...
					case WM_NCLBUTTONDOWN:{
						/*
						 * workaround for half-second
						 * pause when starting to move
						 * window see:
					...
					case WM_INPUT:{
						if (!(win->internal.enabledEvents & RGFW_mousePosChangedFlag) || !(win->internal.holdMouse))
							return DefWindowProcW(hWnd, message, wParam, lParam);
					...
					if (!(win->internal.enabledEvents & RGFW_mouseButtonPressedFlag))
						return DefWindowProcW(hWnd, message, wParam, lParam);
					if (message == WM_XBUTTONDOWN)
						event.button.value = RGFW_mouseMisc1 + (GET_XBUTTON_WPARAM(wParam) == XBUTTON2);
					else
						event.button.value = (message == WM_LBUTTONDOWN) ? (u8) RGFW_mouseLeft :
					...
					if (!(win->internal.enabledEvents & RGFW_mouseButtonReleasedFlag))
						return DefWindowProcW(hWnd, message, wParam, lParam);
					if (message == WM_XBUTTONUP)
						event.button.value = RGFW_mouseMisc1 + (GET_XBUTTON_WPARAM(wParam) == XBUTTON2);
					else
						event.button.value = (message == WM_LBUTTONUP) ? (u8) RGFW_mouseLeft :
					...
					case WM_DROPFILES:{
						...
						if (!(win->internal.enabledEvents & RGFW_dataDrop))
							 return DefWindowProcW(hWnd, message, wParam, lParam);
						 event.type = 0;
						 event.type = RGFW_dataDrop;
						 event.drop.files = _RGFW->files;
...
#define RGFW_LOAD_LIBRARY(x, lib) if (x == NULL) { x = LoadLibraryA(lib); RGFW_ASSERT(x != NULL); }
			...
			/*
			 * we're doing it with magic numbers because some keys
			 * are missing
			 */
			void RGFW_initKeycodesPlatform(void){
			...
			RGFW_window *RGFW_createWindowPlatform(const char *name, RGFW_windowFlags flags, RGFW_window * win){
...
#ifndef __cplusplus
				WNDCLASSW Class = {0};	/* !< Setup the Window
							 * class. */
#else
				WNDCLASSW Class = {};
#endif
				...
				HWND dummyWin = CreateWindowW(Class.lpszClassName, (wchar_t *) wide_name, window_style, win->x, win->y, win->w, win->h, 0, 0, inh, 0);
				 ...
				 RGFW_window_resize(win, win->w, win->h);	/* so WM_GETMINMAXINFO
										 * gets called again */
			...
			void RGFW_window_setFloating(RGFW_window * win, RGFW_bool floating) {
				RGFW_ASSERT(win != NULL);
				if (floating)
					SetWindowPos(win->src.window, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW);
				else
					SetWindowPos(win->src.window, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW);
			}
			...
			u8 RGFW_rgfwToKeyChar(u32 rgfw_keycode) {
				UINT vsc = RGFW_rgfwToApiKey(rgfw_keycode);	/* Should return a
										 * Windows VK_* code */
...
#ifdef __cplusplus
				return RGFW_win32_createMonitor(MonitorFromPoint({
					0, 0
				} ,MONITOR_DEFAULTTOPRIMARY));
#else
				return RGFW_win32_createMonitor(MonitorFromPoint((POINT) {
					0, 0
				} ,MONITOR_DEFAULTTOPRIMARY));
#endif
			}
...
#define RGFW_FREE_LIBRARY(x) if (x != NULL) FreeLibrary(x); x = NULL;
			...
			void RGFW_window_closePlatform(RGFW_window * win) {
				...
				ReleaseDC(win->src.window, win->src.hdc);	/* !< delete device
										 * context */
				DestroyWindow(win->src.window);	/* !< delete window */

				if (win->src.hIconSmall)
					DestroyIcon(win->src.hIconSmall);
				if (win->src.hIconBig)
					DestroyIcon(win->src.hIconBig);
			}
			...
			RGFW_bool RGFW_window_setIconEx(RGFW_window * win, u8 * data, i32 w, i32 h, RGFW_format format, RGFW_icon type) {
				RGFW_ASSERT(win != NULL);
#ifndef RGFW_WIN95
				if (win->src.hIconSmall && (type & RGFW_iconWindow))
					DestroyIcon(win->src.hIconSmall);
				if (win->src.hIconBig && (type & RGFW_iconTaskbar))
					DestroyIcon(win->src.hIconBig);
...
#define WGL_CONTEXT_ROBUST_ACCESS_BIT_ARB 0x00000004
			...
			RGFW_bool RGFW_window_createContextPtr_OpenGL(RGFW_window * win, RGFW_glContext * ctx, RGFW_glHints * hints) {
				...
				/*
				 * try to create the pixel format we want for
				 * OpenGL and then try to create an OpenGL
				 * context for the specified version
				 */
				if (hints->renderer == RGFW_glSoftware)
					pfd.dwFlags |= PFD_GENERIC_FORMAT | PFD_GENERIC_ACCELERATED;

				/*
				 * get pixel format, default to a basic pixel
				 * format
				 */
				...
				if (wglCreateContextAttribsARB != NULL) {
					/*
					 * create OpenGL/WGL context for the
					 * specified version
					 */
					i32 attribs[40];
					RGFW_attribStack stack;
					...
					i32 mask = 0;
					switch (hints->profile) {
						case RGFW_glES:mask |= WGL_CONTEXT_ES_PROFILE_BIT_EXT;
						break;
						case RGFW_glCompatibility:mask |= WGL_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB;
						break;
						case RGFW_glCore:mask |= WGL_CONTEXT_CORE_PROFILE_BIT_ARB;
						break;
					...
					if (RGFW_extensionSupportedPlatform_OpenGL(flushControl, sizeof(flushControl))) {
						if (hints->releaseBehavior == RGFW_glReleaseFlush) {
							RGFW_attribStack_pushAttribs(&stack, WGL_CONTEXT_RELEASE_BEHAVIOR_ARB, WGL_CONTEXT_RELEASE_BEHAVIOR_FLUSH_ARB);
					...
					if (hints->robustness && RGFW_extensionSupportedPlatform_OpenGL(robustness, sizeof(robustness)))
				...
				if (wglCreateContextAttribsARB == NULL || win->src.ctx.native->ctx == NULL) {	/* fall back to a
														 * default context
														 * (probably OpenGL 2 or
														 * something) */
			...
			void RGFW_window_deleteContextPtr_OpenGL(RGFW_window * win, RGFW_glContext * ctx) {
				wglDeleteContext((HGLRC) ctx->ctx);	/* !< delete OpenGL
									 * context */
...
#if defined(RGFW_MACOS)
			/*
			 * based on silicon.h start of cocoa wrapper
			 */
...
#define NSPasteboardTypeString "public.utf8-plain-text"
			...
			typedef RGFW_ENUM(NSInteger, NSOpenGLContextParameter) {
				NSOpenGLContextParameterSwapInterval = 222,	/* 1 param.  0 -> Don't
										 * sync, 1 -> Sync to
										 * vertical retrace     */
					NSOpenGLContextParametectxaceOrder = 235,	/* 1 param.  1 -> Above
											 * Window (default), -1
											 * -> Below Window    */
					NSOpenGLContextParametectxaceOpacity = 236,	/* 1 param.  1-> Surface
											 * is opaque (default),
											 * 0 -> non-opaque   */
					NSOpenGLContextParametectxaceBackingSize = 304,	/* 2 params.
											 * Width/height of
											 * surface backing size              */
					NSOpenGLContextParameterReclaimResources = 308,	/* 0 params.                                                    */
					NSOpenGLContextParameterCurrentRendererID = 309,	/* 1 param.   Retrieves
												 * the current renderer
												 * ID                 */
					NSOpenGLContextParameterGPUVertexProcessing = 310,	/* 1 param.   Currently
												 * processing vertices
												 * with GPU (get)      */
					NSOpenGLContextParameterGPUFragmentProcessing = 311,	/* 1 param.   Currently
												 * processing fragments
												 * with GPU (get)     */
					NSOpenGLContextParameterHasDrawable = 314,	/* 1 param.   Boolean
											 * returned if drawable
											 * is attached          */
					NSOpenGLContextParameterMPSwapsInFlight = 315,	/* 1 param.   Max number
											 * of swaps queued by
											 * the MP GL engine    */

					NSOpenGLContextParameterSwapRectangle API_DEPRECATED("", macos(10.0, 10.14)) = 200,	/* 4 params.  Set or get
																 * the swap rectangle
																 * {x, y, w, h} */
					NSOpenGLContextParameterSwapRectangleEnable API_DEPRECATED("", macos(10.0, 10.14)) = 201,	/* Enable or disable the
																	 * swap rectangle */
					NSOpenGLContextParameterRasterizationEnable API_DEPRECATED("", macos(10.0, 10.14)) = 221,	/* Enable or disable all
																	 * rasterization */
					NSOpenGLContextParameterStateValidation API_DEPRECATED("", macos(10.0, 10.14)) = 301,	/* Validate state for
																 * multi-screen
																 * functionality */
					NSOpenGLContextParametectxaceSurfaceVolatile API_DEPRECATED("", macos(10.0, 10.14)) = 306,	/* 1 param.   Surface
																	 * volatile state */
			};
...
#define NSTrackingEnabledDuringMouseDrag  0x400
			enum {
				NSOpenGLPFAAllRenderers = 1,	/* choose from all
								 * available renderers          */
				NSOpenGLPFATripleBuffer = 3,	/* choose a triple
								 * buffered pixel format        */
				NSOpenGLPFADoubleBuffer = 5,	/* choose a double
								 * buffered pixel format        */
				NSOpenGLPFAAuxBuffers = 7,	/* number of aux buffers                        */
				NSOpenGLPFAColorSize = 8,	/* number of color
								 * buffer bits                  */
				NSOpenGLPFAAlphaSize = 11,	/* number of alpha
								 * component bits               */
				NSOpenGLPFADepthSize = 12,	/* number of depth
								 * buffer bits                  */
				NSOpenGLPFAStencilSize = 13,	/* number of stencil
								 * buffer bits                */
				NSOpenGLPFAAccumSize = 14,	/* number of accum
								 * buffer bits                  */
				NSOpenGLPFAMinimumPolicy = 51,	/* never choose smaller
								 * buffers than
								 * requested  */
				NSOpenGLPFAMaximumPolicy = 52,	/* choose largest
								 * buffers of type
								 * requested     */
				NSOpenGLPFASampleBuffers = 55,	/* number of multi
								 * sample buffers               */
				NSOpenGLPFASamples = 56,	/* number of samples per
								 * multi sample buffer    */
				NSOpenGLPFAAuxDepthStencil = 57,	/* each aux buffer has
									 * its own depth stencil    */
				NSOpenGLPFAColorFloat = 58,	/* color buffers store
								 * floating point pixels    */
				NSOpenGLPFAMultisample = 59,	/* choose multisampling                         */
				NSOpenGLPFASupersample = 60,	/* choose supersampling                         */
				NSOpenGLPFASampleAlpha = 61,	/* request alpha
								 * filtering                      */
				NSOpenGLPFARendererID = 70,	/* request renderer by
								 * ID                       */
				NSOpenGLPFANoRecovery = 72,	/* disable all failure
								 * recovery systems         */
				NSOpenGLPFAAccelerated = 73,	/* choose a hardware
								 * accelerated renderer       */
				NSOpenGLPFAClosestPolicy = 74,	/* choose the closest
								 * color buffer to
								 * request   */
				NSOpenGLPFABackingStore = 76,	/* back buffer contents
								 * are valid after swap    */
				NSOpenGLPFAScreenMask = 84,	/* bit mask of supported
								 * physical screens       */
				NSOpenGLPFAAllowOfflineRenderers = 96,	/* allow use of offline
									 * renderers               */
				NSOpenGLPFAAcceleratedCompute = 97,	/* choose a hardware
									 * accelerated compute
									 * device */
				NSOpenGLPFAOpenGLProfile = 99,	/* specify an OpenGL
								 * Profile to use             */
				NSOpenGLProfileVersionLegacy = 0x1000,	/* The requested profile
									 * is a legacy
									 * (pre-OpenGL 3.0)
									 * profile. */
				NSOpenGLProfileVersion3_2Core = 0x3200,	/* The 3.2 Profile of
									 * OpenGL */
				NSOpenGLProfileVersion4_1Core = 0x3200,	/* The 4.1 profile of
									 * OpenGL */
				NSOpenGLPFAVirtualScreenCount = 128,	/* number of virtual
									 * screens in this
									 * format     */
				NSOpenGLPFAStereo = 6,
			...
			typedef RGFW_ENUM(u32, NSEventType) {	/* various types of
								 * events */
			...
			typedef RGFW_ENUM(NSUInteger, NSBitmapFormat) {
				NSBitmapFormatAlphaFirst = 1 << 0,	/* 0 means is alpha last
									 * (RGBA, CMYKA, etc.) */
					NSBitmapFormatAlphaNonpremultiplied = 1 << 1,	/* 0 means is
											 * premultiplied */
...
#else				/* __i386__ */
			/*
			 * x86 just uses abi_objc_msgSend_fpret and (NSColor
			 * *)objc_msgSend_id respectively
			 */
...
#define NSRetain(obj) objc_msgSend_void((id)obj, sel_registerName("retain"))
			...
			/*
			 * NOTE(EimaMei): Fixes the constant clicking when the
			 * app is running under a terminal.
			 */
			...
			bool performDragOperation(id self, SEL sel, id sender) {
				...
				/*
				 * Get the types of data available on the
				 * pasteboard
				 */
...
#include <IOKit/IOKitLib.h>
			...
			static void RGFW__osxWindowDeminiaturize(id self, SEL sel) {
				...
				if (win == NULL)
				 ...
				 win->internal.flags |= RGFW_windowMinimize;
				if (!(win->internal.enabledEvents & RGFW_windowMinimizedFlag))
					 return;
				 RGFW_eventQueuePushEx(e.type = RGFW_windowRestored;
				 e.common.win = win);
				 RGFW_windowRestoredCallback(win, win->x, win->y, win->w, win->h);
			...
			static void RGFW__osxDidWindowResize(id self, SEL _cmd, id notification) {
				...
				NSRect frame;
				if (win->src.view)
					 frame = ((NSRect(*) (id, SEL)) abi_objc_msgSend_stret) ((id) win->src.view, sel_registerName("frame"));
				else
					 return;
				...
				if ((i32) mon.mode.w == win->w && (i32) mon.mode.h - 102 <= win->h) {
					...
					win->internal.flags &= ~(u32) RGFW_windowMaximize;
					if (!(win->internal.enabledEvents & RGFW_windowRestoredFlag))
						return;
					RGFW_eventQueuePushEx(e.type = RGFW_windowRestored;
					e.common.win = win);
					RGFW_windowRestoredCallback(win, win->x, win->y, win->w, win->h);
			...
			static void RGFW__osxViewDidChangeBackingProperties(id self, SEL _cmd) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_scaleUpdatedFlag))
					return;

				RGFW_monitor mon = RGFW_window_getMonitor(win);
				 RGFW_scaleUpdatedCallback(win, mon.scaleX, mon.scaleY);
				 RGFW_eventQueuePushEx(e.type = RGFW_scaleUpdated;
				 e.scale.x = mon.scaleX;
				 e.scale.y = mon.scaleY;
				 e.common.win = win);
			}

			static BOOL RGFW__osxWantsUpdateLayer(id self, SEL _cmd) {
				RGFW_UNUSED(self);
				RGFW_UNUSED(_cmd);
				return YES;
			}
			...
			static void RGFW__osxUpdateLayer(id self, SEL _cmd) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_windowRefreshFlag))
					return;
				 RGFW_windowRefreshCallback(win);
			}
			...
			static void RGFW__osxMouseEntered(id self, SEL _cmd, id event) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_mouseEnterFlag))
					return;
			...
			void RGFW__osxMouseExited(id self, SEL _cmd, id event) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_mouseLeaveFlag))
					return;
			...
			void RGFW__osxKeyDown(id self, SEL _cmd, id event) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_keyPressedFlag))
					return;
			...
			void RGFW__osxKeyUp(id self, SEL _cmd, id event) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_keyReleasedFlag))
					return;
			...
			void RGFW__osxFlagsChanged(id self, SEL _cmd, id event) {
				...
				if (!(win->internal.enabledEvents & (RGFW_BIT(e.type))))
					 return;
				 RGFW_eventQueuePush(&e);
			...
			void RGFW__osxMouseMoved(id self, SEL _cmd, id event) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_windowMovedFlag))
					return;
			...
			void RGFW__osxMouseDown(id self, SEL _cmd, id event) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_mouseButtonPressedFlag))
					return;

				RGFW_event e;
				u32 buttonNumber = (u32) ((u32(*) (id, SEL)) objc_msgSend) (event, sel_registerName("buttonNumber"));
				switch (buttonNumber) {
					case 0:e.button.value = RGFW_mouseLeft;
					break;
					case 1:e.button.value = RGFW_mouseRight;
					break;
					case 2:e.button.value = RGFW_mouseMiddle;
					break;
			...
			void RGFW__osxMouseUp(id self, SEL _cmd, id event) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_mouseButtonReleasedFlag))
					return;
			...
			void RGFW__osxScrollWheel(id self, SEL _cmd, id event) {
				...
				RGFW_window *win = NULL;
				 object_getInstanceVariable(self, "RGFW_window", (void **)&win);
				if (win == NULL || !(win->internal.enabledEvents & RGFW_mouseScroll))
					return;
			...
			i32 RGFW_initPlatform(void){
				...
				/*
				 * NOTE(EimaMei): Fixes the 'Boop' sfx from
				 * constantly playing each time you click a
				 * key. Only a problem when running in the
				 * terminal.
				 */
			...
			void RGFW_pollEvents(void){
				/*
				 * TODO look to see if all these events can be
				 * replaced with callbacks callbacks seem to
				 * give better info on mac's api
				 */
			...
			void RGFW_window_move(RGFW_window * win, i32 x, i32 y) {
				...
				((id) win->src.window, sel_registerName("setFrame:display:animate:"), (NSRect) {{
						(double)win->x, (double)win->y
				} ,{
					(double)win->w, (double)win->h
				}
				} ,true, true);
			}
			...
			void RGFW_window_resize(RGFW_window * win, i32 w, i32 h) {
				...
				float offset = (float)(frame.size.height - content.size.height);
				 ...
				 ((void (*) (id, SEL, CGRect))objc_msgSend) ((id) win->src.view, sel_registerName("setFrame:"), (NSRect) {{
						0, 0
				} ,{
					(double)win->w, (double)win->h
				}
				 ...
				 ((id) win->src.window, sel_registerName("setFrame:display:animate:"), (NSRect) {{
					...
					(double)win->w, (double)win->h + (double)offset
				}
				} ,true, true);
			}
			...
			void RGFW_window_setFullscreen(RGFW_window * win, RGFW_bool fullscreen) {
				RGFW_ASSERT(win != NULL);
				if (fullscreen && (win->internal.flags & RGFW_windowFullscreen))
					return;
				if (!fullscreen && !(win->internal.flags & RGFW_windowFullscreen))
					return;
			...
			void RGFW_window_setFloating(RGFW_window * win, RGFW_bool floating) {
				RGFW_ASSERT(win != NULL);
				if (floating)
					objc_msgSend_void_id(win->src.window, sel_registerName("setLevel:"), kCGFloatingWindowLevelKey);
				else
					objc_msgSend_void_id(win->src.window, sel_registerName("setLevel:"), kCGNormalWindowLevelKey);
			}
			...
			void RGFW_window_setAspectRatio(RGFW_window * win, i32 w, i32 h) {
				...
				((id) win->src.window, sel_registerName("setContentAspectRatio:"), (NSSize) {
					(CGFloat) w, (CGFloat) h
				});
			}

			void RGFW_window_setMinSize(RGFW_window * win, i32 w, i32 h) {
				((void (*) (id, SEL, NSSize))objc_msgSend) ((id) win->src.window, sel_registerName("setMinSize:"), (NSSize) {
					(CGFloat) w, (CGFloat) h
				});
			}
			...
			void RGFW_window_setMaxSize(RGFW_window * win, i32 w, i32 h) {
				...
				((void (*) (id, SEL, NSSize))objc_msgSend)
				((id) win->src.window, sel_registerName("setMaxSize:"), (NSSize) {
					(CGFloat) w, (CGFloat) h
				});
			}
			...
			void RGFW_captureCursor(RGFW_window * win) {
				...
				CGWarpMouseCursorPosition((CGPoint) {
					(CGFloat) (win->x + (win->w / 2)), (CGFloat) (win->y + (win->h / 2))
				});
			...
			/*
			 * MacOS OpenGL API spares us yet again (there are no
			 * extensions)
			 */
			RGFW_bool RGFW_extensionSupportedPlatform_OpenGL(const char *extension, size_t len){
				RGFW_UNUSED(extension);
				RGFW_UNUSED(len);
				return RGFW_FALSE;
			}
			...
			RGFW_bool RGFW_window_createContextPtr_OpenGL(RGFW_window * win, RGFW_glContext * ctx, RGFW_glHints * hints) {
				...
				size_t render_type_index = 0;
				{
					...
					if (hints->samples) {
						...
						RGFW_attribStack_pushAttribs(&stack, NSOpenGLPFASamples, hints->samples);
						 ...
						 RGFW_attribStack_pushAttribs(&stack, NSOpenGLPFASampleBuffers, 0);
					...
					if (hints->stereo]
						) RGFW_attribStack_pushAttrib(&stack, NSOpenGLPFAStereo);
#endif

					/*
					 * macOS has the surface attribs and
					 * the OpenGL attribs connected for
					 * some reason maybe this is to give
					 * macOS more control to limit
					 * openGL/the OpenGL version?
					 */
				...
				/*
				 * the pixel format can be passed directly to
				 * OpenGL context creation to create a context
				 * this is because the format also includes
				 * information about the OpenGL version (which
				 * may be a bad thing)
				 */
...
#define NSOpenGLCPSurfaceOpacity 236
			...
			WGPUSurface RGFW_window_createSurface_WebGPU(RGFW_window * window, WGPUInstance instance) {
				...
				if (metalLayer == NULL) {
				 ...
				 layer = metalLayer;	/* Use the newly created
							 * layer */

				/*
				 * At this point, 'layer' should be a valid
				 * CAMetalLayer*
				 */
				WGPUSurfaceSourceMetalLayer fromMetal = {0};
				 fromMetal.chain.sType = WGPUSType_SurfaceSourceMetalLayer;
#ifdef  __OBJC__
				 fromMetal.layer = (__bridge CAMetalLayer *) layer;	/* Use __bridge for ARC
											 * compatibility if
											 * mixing C/Obj-C */
#else
			...
			EM_BOOL Emscripten_on_resize(int eventType, const EmscriptenUiEvent * E, void *userData){
				...
				if (!(_RGFW->root->internal.enabledEvents & RGFW_windowResizedFlag))
					return EM_TRUE;
			...
			EM_BOOL Emscripten_on_focusin(int eventType, const EmscriptenFocusEvent * E, void *userData){
				...
				if (!(_RGFW->root->internal.enabledEvents & RGFW_focusInFlag))
					return EM_TRUE;
			...
			EM_BOOL Emscripten_on_focusout(int eventType, const EmscriptenFocusEvent * E, void *userData){
				...
				if (!(_RGFW->root->internal.enabledEvents & RGFW_focusOutFlag))
					return EM_TRUE;
			...
			EM_BOOL Emscripten_on_mousemove(int eventType, const EmscriptenMouseEvent * E, void *userData){
				...
				if (!(_RGFW->root->internal.enabledEvents & RGFW_mousePosChangedFlag))
					return EM_TRUE;

				RGFW_eventQueuePushEx(e.type = RGFW_mousePosChanged;
				e.mouse.x = E->targetX;
				e.mouse.y = E->targetY;
				e.mouse.vecX = E->movementX;
				e.mouse.vecY = E->movementY;
				e.common.win = _RGFW->root);
			...
			EM_BOOL Emscripten_on_mousedown(int eventType, const EmscriptenMouseEvent * E, void *userData){
				...
				if (!(_RGFW->root->internal.enabledEvents & RGFW_mouseButtonPressedFlag))
					return EM_TRUE;
			...
			EM_BOOL Emscripten_on_mouseup(int eventType, const EmscriptenMouseEvent * E, void *userData){
				...
				if (!(_RGFW->root->internal.enabledEvents & RGFW_mouseButtonReleasedFlag))
					return EM_TRUE;
			...
			EM_BOOL Emscripten_on_wheel(int eventType, const EmscriptenWheelEvent * E, void *userData){
				...
				if (!(_RGFW->root->internal.enabledEvents & RGFW_mouseScrollFlag))
					return EM_TRUE;
...
if (*((u32 *) key) == *((u32 *) "Tab"))
				...
				if (!(press ? (_RGFW->root->internal.enabledEvents & RGFW_keyPressedFlag) : (_RGFW->root->internal.enabledEvents & RGFW_keyReleasedFlag)))
					 return;
			...
			void EMSCRIPTEN_KEEPALIVE Emscripten_onDrop(size_t count) {
				...
				if (!(_RGFW->root->internal.enabledEvents & RGFW_dataDropFlag))
					return;
			...
			void EMSCRIPTEN_KEEPALIVE RGFW_makeSetValue(size_t index, char *file){
				/*
				 * This seems like a terrible idea, don't
				 * replicate this unless you hate yourself or
				 * the OS
				 */
				/*
...
#include <stdio.h>
			...
			RGFW_ssize_t RGFW_readClipboardPtr(char *str, size_t strCapacity){
				...
				RGFW_UNUSED(strCapacity);
				/*
				 * placeholder code for later I'm not sure if
				 * this is possible do the the async stuff
				 */
				return 0;
			}
			...
			void *RGFW_getCurrentContext_OpenGL(void){
				return (void *)emscripten_webgl_get_current_context();
			}
			...
			/*
			 * RGFW function pointer backend, made to allow you to
			 * compile for Wayland but fallback to X11
			 */
#ifdef RGFW_DYNAMIC
			...
			RGFW_bool RGFW_createSurfacePtr(u8 * data, i32 w, i32 h, RGFW_format format, RGFW_surface * surface) {
				return RGFW_api.createSurfacePtr(data, w, h, format, surface);
			}
			void RGFW_surface_freePtr(RGFW_surface * surface) {
				RGFW_api.surface_freePtr(surface);
			}
			void RGFW_freeMouse(RGFW_mouse * mouse) {
				RGFW_api.freeMouse(mouse);
			}
			void RGFW_window_blitSurface(RGFW_window * win, RGFW_surface * surface) {
				RGFW_api.window_blitSurface(win, surface);
			}
			void RGFW_window_setBorder(RGFW_window * win, RGFW_bool border) {
				RGFW_api.window_setBorder(win, border);
			}
			void RGFW_releaseCursor(RGFW_window * win) {
				RGFW_api.releaseCursor(win);
			}
			void RGFW_captureCursor(RGFW_window * win) {
				RGFW_api.captureCursor(win);
			}
			RGFW_window *RGFW_createWindowPlatform(const char *name, RGFW_windowFlags flags, RGFW_window * win){
				RGFW_init();
				return RGFW_api.createWindowPlatform(name, flags, win);
			}
			RGFW_bool RGFW_getGlobalMouse(i32 * x, i32 * y) {
				return RGFW_api.getGlobalMouse(x, y);
			}
			u8 RGFW_rgfwToKeyChar(u32 key) {
				return RGFW_api.rgfwToKeyChar(key);
			}
			void RGFW_pollEvents(void){
				RGFW_api.pollEvents();
			}
			void RGFW_window_move(RGFW_window * win, i32 x, i32 y) {
				RGFW_api.window_move(win, x, y);
			}
			void RGFW_window_resize(RGFW_window * win, i32 w, i32 h) {
				RGFW_api.window_resize(win, w, h);
			}
			void RGFW_window_setAspectRatio(RGFW_window * win, i32 w, i32 h) {
				RGFW_api.window_setAspectRatio(win, w, h);
			}
			void RGFW_window_setMinSize(RGFW_window * win, i32 w, i32 h) {
				RGFW_api.window_setMinSize(win, w, h);
			}
			void RGFW_window_setMaxSize(RGFW_window * win, i32 w, i32 h) {
				RGFW_api.window_setMaxSize(win, w, h);
			}
			void RGFW_window_maximize(RGFW_window * win) {
				RGFW_api.window_maximize(win);
			}
			void RGFW_window_focus(RGFW_window * win) {
				RGFW_api.window_focus(win);
			}
			void RGFW_window_raise(RGFW_window * win) {
				RGFW_api.window_raise(win);
			}
			void RGFW_window_setFullscreen(RGFW_window * win, RGFW_bool fullscreen) {
				RGFW_api.window_setFullscreen(win, fullscreen);
			}
			void RGFW_window_setFloating(RGFW_window * win, RGFW_bool floating) {
				RGFW_api.window_setFloating(win, floating);
			}
			void RGFW_window_setOpacity(RGFW_window * win, u8 opacity) {
				RGFW_api.window_setOpacity(win, opacity);
			}
			void RGFW_window_minimize(RGFW_window * win) {
				RGFW_api.window_minimize(win);
			}
			void RGFW_window_restore(RGFW_window * win) {
				RGFW_api.window_restore(win);
			}
			RGFW_bool RGFW_window_isFloating(RGFW_window * win) {
				return RGFW_api.window_isFloating(win);
			}
			void RGFW_window_setName(RGFW_window * win, const char *name){
				RGFW_api.window_setName(win, name);
			}

#ifndef RGFW_NO_PASSTHROUGH
			void RGFW_window_setMousePassthrough(RGFW_window * win, RGFW_bool passthrough) {
				RGFW_api.window_setMousePassthrough(win, passthrough);
			}
#endif

			RGFW_bool RGFW_window_setIconEx(RGFW_window * win, u8 * data, i32 w, i32 h, RGFW_format format, u8 type) {
				return RGFW_api.window_setIconEx(win, data, w, h, format, type);
			}
			RGFW_mouse *RGFW_loadMouse(u8 * data, i32 w, i32 h, RGFW_format format) {
				return RGFW_api.loadMouse(data, w, h, format);
			}
			void RGFW_window_setMouse(RGFW_window * win, RGFW_mouse * mouse) {
				RGFW_api.window_setMouse(win, mouse);
			}
			void RGFW_window_moveMouse(RGFW_window * win, i32 x, i32 y) {
				RGFW_api.window_moveMouse(win, x, y);
			}
			RGFW_bool RGFW_window_setMouseDefault(RGFW_window * win) {
				return RGFW_api.window_setMouseDefault(win);
			}
			RGFW_bool RGFW_window_setMouseStandard(RGFW_window * win, u8 mouse) {
				return RGFW_api.window_setMouseStandard(win, mouse);
			}
			void RGFW_window_hide(RGFW_window * win) {
				RGFW_api.window_hide(win);
			}
			void RGFW_window_show(RGFW_window * win) {
				RGFW_api.window_show(win);
			}
			RGFW_ssize_t RGFW_readClipboardPtr(char *str, size_t strCapacity){
				return RGFW_api.readClipboardPtr(str, strCapacity);
			}
			void RGFW_writeClipboard(const char *text, u32 textLen){
				RGFW_api.writeClipboard(text, textLen);
			}
			RGFW_bool RGFW_window_isHidden(RGFW_window * win) {
				return RGFW_api.window_isHidden(win);
			}
			RGFW_bool RGFW_window_isMinimized(RGFW_window * win) {
				return RGFW_api.window_isMinimized(win);
			}
			RGFW_bool RGFW_window_isMaximized(RGFW_window * win) {
				return RGFW_api.window_isMaximized(win);
			}
			RGFW_monitor *RGFW_getMonitors(size_t * len) {
				return RGFW_api.getMonitors(len);
			}
			RGFW_monitor RGFW_getPrimaryMonitor(void){
				return RGFW_api.getPrimaryMonitor();
			}
			RGFW_bool RGFW_monitor_requestMode(RGFW_monitor mon, RGFW_monitorMode mode, RGFW_modeRequest request) {
				return RGFW_api.monitor_requestMode(mon, mode, request);
			}
			RGFW_monitor RGFW_window_getMonitor(RGFW_window * win) {
				return RGFW_api.window_getMonitor(win);
			}
			void RGFW_window_closePlatform(RGFW_window * win) {
				RGFW_api.window_closePlatform(win);
			}

#ifdef RGFW_OPENGL
			RGFW_bool RGFW_extensionSupportedPlatform_OpenGL(const char *extension, size_t len){
				return RGFW_api.extensionSupportedPlatform_OpenGL(extension, len);
			}
			RGFW_proc RGFW_getProcAddress_OpenGL(const char *procname){
				return RGFW_api.getProcAddress_OpenGL(procname);
			}
			RGFW_bool RGFW_window_createContextPtr_OpenGL(RGFW_window * win, RGFW_glContext * ctx, RGFW_glHints * hints) {
				return RGFW_api.window_createContextPtr_OpenGL(win, ctx, hints);
			}
			void RGFW_window_deleteContextPtr_OpenGL(RGFW_window * win, RGFW_glContext * ctx) {
				RGFW_api.window_deleteContextPtr_OpenGL(win, ctx);
			}
			void RGFW_window_makeCurrentContext_OpenGL(RGFW_window * win) {
				RGFW_api.window_makeCurrentContext_OpenGL(win);
			}
			void *RGFW_getCurrentContext_OpenGL(void){
				return RGFW_api.getCurrentContext_OpenGL();
			}
			void RGFW_window_swapBuffers_OpenGL(RGFW_window * win) {
				RGFW_api.window_swapBuffers_OpenGL(win);
			}
			void RGFW_window_swapInterval_OpenGL(RGFW_window * win, i32 swapInterval) {
				RGFW_api.window_swapInterval_OpenGL(win, swapInterval);
			}
#endif

#ifdef RGFW_WEBGPU
			WGPUSurface RGFW_window_createSurface_WebGPU(RGFW_window * window, WGPUInstance instance) {
				return RGFW_api.window_createSurface_WebGPU(window, instance);
			}
#endif
#endif				/* RGFW_DYNAMIC */

			/*
			 * start of X11 AND wayland defines this allows a
			 * single executable to support x11 AND wayland falling
			 * back to x11 if wayland fails to initalize
			 */
#if defined(RGFW_WAYLAND) && defined(RGFW_X11)
			void RGFW_load_X11(void){
